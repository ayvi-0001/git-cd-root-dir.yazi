...
---@param str: string
local isempty = function(str) return str == nil or str == '' end
...
local _url_eq = function(url1, url2)
  if type(url1) == 'string' then url1 = Url(url1) end
  if type(url2) == 'string' then url2 = Url(url2) end
...
function F:entry(job)
  ...
  if not _url_eq(cwd, latest_cwd) then return end

  -- Before confirming we're not in a repo,
  -- check to see if we are in the .git folder, since the result of
  ...
  local function recurse_dir_search(path)
    if path:ends_with('.git') then -- Found the .git folder.
      return path:parent()
